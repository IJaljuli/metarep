...
```{r remove bounds, fig.width=8.5, fig.height=3}
...
```{r forest CD007077, fig.width=9, fig.height=3}
m7077.ra <- metarep(x = m7077 , u = 2 , common.effect = F ,t = 0.05 ,report.u.max = T)
...
"We cannot rule out the possibility that this result is critically based on a single study ( $r − value = 1$). Moreover, the results are inconsistent, since with $95\%$ confidence, we conlclude that at least one study had an increased effect and at least one study had a decreased effect."
...
```{r forest CD6823, fig.width=10, fig.height=3}
m6823.ra <- metarep(x = m6823 , u = 2 , common.effect = F ,t = 0.05 ,report.u.max = T)
...
```{r forest CD3366, fig.width=10, fig.height=8}
m3366.ra <- metarep(x = m3366 , u = 2 , common.effect = F ,t = 0.05 ,report.u.max = T)
